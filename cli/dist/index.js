...
import { rbacStatePda, rolePda, permissionPda, userRolePda, rolePermissionPda, explorerUrl, shortKey, } from './helpers.js';
// ────────────────────────────────────────────────────────────────────────────
// Config
// ────────────────────────────────────────────────────────────────────────────
const IDL_PATH = path.resolve(path.dirname(new URL(import.meta.url).pathname), '../../target/idl/rbac.json');
function loadIdl() {
...
function banner() {
    console.log(boxen(BRAND.bold('RBAC ON-CHAIN ENGINE') + '\n' +
...
cli
    ...
    .action(async (name, resource, action) => {
    const { program, wallet } = getProgram();
...
cli
    ...
    .action(async (roleName, permName) => {
...
cli
    ...
    .action(async (userPubkey, roleName) => {
    const { program, wallet, programId } = getProgram();
...
cli
    ...
    .action(async (userPubkey, roleName, permName) => {
    const { program, wallet, programId } = getProgram();
    ...
    try {
        ...
        spinner.stop();
        console.log('');
        console.log(boxen(SUCCESS.bold('ACCESS GRANTED ✓') + '\n\n' +
        ...
        spinner.stop();
        console.log('');
        console.log(boxen(ERROR.bold('ACCESS DENIED ✗') + '\n\n' +
            DIM('User:       ') + MONO(shortKey(userPubkey)) + '\n' +
            DIM('Role:       ') + BRAND(roleName) + '\n' +
            DIM('Permission: ') + BRAND(permName), {
            padding: { top: 0, bottom: 0, left: 2, right: 2 },
...
cli
    ...
    try {
        ...
        const acct = await program.account.rbacState.fetch(state);
        spinner.stop();
        console.log('');
        console.log(boxen(BRAND.bold('SYSTEM STATUS') + '\n\n' +
...
cli
    ...
    .action(async () => {
    const { program, wallet, programId } = getProgram();
    console.log(boxen(BRAND.bold('HOSPITAL RECORDS DEMO') + '\n' +
    ...
    const steps = [
            ...
            label: '1. Initialize RBAC', fn: async () => {
                try {
                    ...
                    if (e.message?.includes('already in use'))
                        return 'SKIP_ALREADY_INIT';
                    throw e;
                }
            }
        },
        {
            label: '2. Create role: doctor', fn: () => program.methods.createRole('doctor')
                .accounts({ authority: wallet.publicKey })
                .signers([wallet]).rpc()
        },
        {
            label: '3. Create role: nurse', fn: () => program.methods.createRole('nurse')
                .accounts({ authority: wallet.publicKey })
                .signers([wallet]).rpc()
        },
        {
            label: '4. Create permission: read_medical', fn: () => program.methods.createPermission('read_medical', 'medical_record', 'read')
                .accounts({ authority: wallet.publicKey })
                .signers([wallet]).rpc()
        },
        {
            label: '5. Create permission: write_medical', fn: () => program.methods.createPermission('write_medical', 'medical_record', 'write')
                .accounts({ authority: wallet.publicKey })
                .signers([wallet]).rpc()
        },
        {
            label: '6. Link: doctor → read_medical', fn: () => program.methods.assignPermissionToRole()
                .accountsPartial({
        ...
        {
            label: '7. Link: doctor → write_medical', fn: () => program.methods.assignPermissionToRole()
                .accountsPartial({
                role: rolePda('doctor', programId),
        ...
        {
            label: '8. Link: nurse → read_medical (only)', fn: () => program.methods.assignPermissionToRole()
                .accountsPartial({
        ...
        {
            label: '9. Assign: self → doctor', fn: () => program.methods.assignRoleToUser()
                .accountsPartial({
                role: rolePda('doctor', programId),
